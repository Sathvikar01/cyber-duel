...
export const MOVE_DATA = Object.freeze({
  ...
  // Walk is not an attack but it lives in MOVE_DATA so that
  // getMoveDuration() returns a non-zero value, which is what
  // updateAnimation() uses to advance the animProgress through the
  // walk cycle. A full step-pair takes ~0.6s. The animation
  // continuously loops while the fighter has movement input (or
  // while the AI is in an approach/retreat state).
  walk: {
    damage: 0,
    staminaCost: 0,
    range: 0,
